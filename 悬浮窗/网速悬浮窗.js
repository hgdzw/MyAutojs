TTrafficStats = android.net.TrafficStats;
...
function textpaint(str) {
    str = str.split("\n");
    var bitmap = Bitmap.createBitmap(215, 155, Bitmap.Config.ARGB_8888);

    var canvas = new Canvas(bitmap);
    canvas.drawARGB(122, 0, 0, 0);
    var paint = new Paint();
    //圆盘背景
    paint.setARGB(0, 0, 0, 0);//白色画笔
    paint.setStyle(Paint.Style.FILL);//空心样式  
    paint.setFilterBitmap(true);//抗锯齿
    paint.setFakeBoldText(true);//加粗
    paint.setHinting(20);//字体微调
...
while (true) {
    sleep(1000);
} rafficStats = android.net.TrafficStats;
importClass(android.graphics.Paint);
Canvas = android.graphics.Canvas;
importClass(android.graphics.Bitmap);
